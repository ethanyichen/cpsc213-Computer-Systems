...
uthread_sem_t stack_lock;
uthread_sem_t not_empty;
uthread_sem_t not_full;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
	...
	uthread_sem_wait (not_full);
	uthread_sem_wait (stack_lock);
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
	uthread_sem_signal (stack_lock);
	uthread_sem_signal (not_empty);
  }
  return NULL;
}
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
	  uthread_sem_wait (not_empty);
	  uthread_sem_wait (stack_lock);
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;  
	uthread_sem_signal (stack_lock);
	uthread_sem_signal (not_full);
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  stack_lock = uthread_sem_create(1);
  not_empty = uthread_sem_create(0);
  not_full = uthread_sem_create(MAX_ITEMS);
