...
int exec() {
    int cont = 1;
    int addr, val;
    void* jt[] = { &&L0, &&L1, &&L2, &&L3, &&L4, &&DEFAULT, &&L6, &&L7, &&L8, &&L9, &&La, &&Lb, &&Lc, &&DEFAULT, &&DEFAULT, &&Lf };
	void* jtALU[] = { &&L60, &&L61, &&L62, &&L63, &&L64, &&L65, &&L66, &&L67, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&L6f };
    if (insOpCode < 0x0 || insOpCode > 0xf) {
        goto ALU_DEFAULT;
    }
    goto *jt[insOpCode];
    ...
    L1:	addr = (insOp0 << 2) + reg [insOp1];
		reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
		goto CONT;
    L2: addr = reg [insOp0] + (reg [insOp1] << 2);
		reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
		goto CONT;
    L3: addr = (insOp1 << 2) + reg [insOp2];
        val  = reg [insOp0];
        ...
        mem [addr+3] = val       & 0xff;
		goto CONT;
    L4: addr = reg [insOp1] + (reg [insOp2] << 2);
        val  = reg [insOp0];
        mem [addr]   = val >> 24 & 0xff;
        mem [addr+1] = val >> 16 & 0xff;
        mem [addr+2] = val >>  8 & 0xff;
        mem [addr+3] = val       & 0xff;
		goto CONT;
    L6: if (insOp0 < 0x0 || insOp0 > 0xf) {
			goto ALU_DEFAULT;
		}
		goto *jtALU[insOp0];
		L60: reg [insOp2] = reg [insOp1];
				goto ALU_CONT;
		L61: reg [insOp2] = reg [insOp1] + reg [insOp2];
				goto ALU_CONT;
		L62: reg [insOp2] = reg [insOp1] & reg [insOp2];
				goto ALU_CONT;
		L63: reg [insOp2] = reg [insOp2] + 1;
				goto ALU_CONT;
		L64: reg [insOp2] = reg [insOp2] + 4;
				goto ALU_CONT;
		L65: reg [insOp2] = reg [insOp2] - 1;
				goto ALU_CONT;
		L66: reg [insOp2] = reg [insOp2] -4;
				goto ALU_CONT;
		L67: reg [insOp2] = ~ reg [insOp2];
				goto ALU_CONT;
		L6f: reg [insOp2] = pc + (insOp1 << 1);
				goto ALU_CONT;
        ALU_DEFAULT:
    ...
    Lb: pc = insOpExt;
		goto CONT;
    Lc: pc = (((unsigned short) insOpImm) << 1) + reg [insOp0];
		goto CONT;
    Lf: if (insOp0 == 0)
          cont = 0;
        else if (insOp0 == 1) {
