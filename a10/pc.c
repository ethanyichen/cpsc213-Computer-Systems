...
uthread_mutex_t mx;
uthread_cond_t noItems, itemsFull;
...
void *producer(void *v) {
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        ...
        while(items >= MAX_ITEMS){
            uthread_cond_wait(itemsFull);
        }
        items += 1;
        assert(items >= 0 && items <= MAX_ITEMS);
        histogram[items]++;
        uthread_cond_signal(noItems);
...
void *consumer(void *v) {
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        uthread_mutex_lock(mx);
        while(items <= 0 ){
            uthread_cond_wait(noItems);
        }
        items -= 1;
        assert(items >= 0 && items <= MAX_ITEMS);
        histogram[items]++;
        uthread_cond_signal(itemsFull);
        uthread_mutex_unlock(mx);
    }
    return NULL;
}
...
int main(int argc, char **argv) {
    ...
    itemsFull = uthread_cond_create(mx);
    noItems = uthread_cond_create(mx);
    // start the threads
